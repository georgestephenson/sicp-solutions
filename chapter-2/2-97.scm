  ...
  (define (gcd-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        ...
        (error "Polys not in same var -- GCD-POLY"
              (list p1 p2))))
  (define (integerise-terms n d)
    (define (int-factor o1 q)
      (let ((o2 (order (car q)))
            (c (coeff (car q))))
        (expt c (+ 1 o1 (- o2)))))
    (let ((q (gcd-terms n d)))
      (let ((x (int-factor (max (order (car n)) (order (car d)))
                           q)))
        (list (mul-terms-by-int n x) (mul-terms-by-int d x)))))
  (define (reduce-terms n d)
    (let* ((a (integerise-terms n d))
          (g (gcd-terms (car a) (cadr a)))
          (quot-n (car (div-terms (car a) g)))
          (quot-d (car (div-terms (cadr a) g))))
      (if (< (coeff (first-term quot-d)) 0)
          (list (mul-terms-by-int quot-n -1)
                (mul-terms-by-int quot-d -1))
          (list quot-n quot-d))))
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((p1p2-terms (reduce-terms (term-list p1)
                                 (term-list p2))))
          (list (tag (make-poly (variable p1) (car p1p2-terms)))
                (tag (make-poly (variable p1) (cadr p1p2-terms)))))
...
rf1
;Value: (rational (polynomial x (1 1) (0 1)) polynomial x (3 1) (0 -1))

(define rf2 (make-rational p3 p4))
rf2
;Value: (rational (polynomial x (1 1)) polynomial x (2 1) (0 -1))

(add rf1 rf2)
;Value: (rational (polynomial x (3 1) (2 2) (1 3) (0 1)) polynomial x (4 1) (3 1) (1 -1) (0 -1))